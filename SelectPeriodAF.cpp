        ...
        : TForm(Owner), date1(dt1), date2(dt2)
{
  ...
  FormStorage->IniSection="SelectPeriodAF";
  ...
  { unsigned short y1,m1,d1, y2,m2,d2;
    ...
    { if(m1==m2)
      { if(d1==d2)
        ...
        else if(d1==1 && d2==SaleBookDM->LastDay(y2,m2))
      ...
      { if(date1==TDateTime(y1,1,1) && date2==TDateTime(y2,m2,SaleBookDM->LastDay(y2,m2)))
        { MonthButton->Checked=true;
...
void __fastcall TSelectPeriodForm::JXPBOKClick(TObject *Sender)
{
  ...
  else
  {
   ...
   if (y1!=y2)
   {
   MessageDlg("ѕериод должен быть только одного года или мес€ца!!! ",mtWarning,TMsgDlgButtons() << mbOK,0);
   return;
   }
   if (m1!=m2)
   {
